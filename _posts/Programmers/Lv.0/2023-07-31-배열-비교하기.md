...
> 문제를 직접 풀어보실 경우 [**여기**](https://school.programmers.co.kr/learn/courses/30/lessons/181856){:target="_blank"}를 클릭해 주세요.
{: .prompt-tip }
...
function solution(arr1, arr2) {
  ...
  if (arr1.length > arr2.length || (arr1.length === arr2.length && sumArr1 > sumArr2)) {
    return 1;
  } else if (arr1.length < arr2.length || (arr1.length === arr2.length && sumArr1 < sumArr2)) {
    return -1;
  } else {
    return 0;
  }
