...
> 문제를 직접 풀어보실 경우 [**여기**](https://school.programmers.co.kr/learn/courses/30/lessons/181854){:target="_blank"}를 클릭해 주세요.
{: .prompt-tip }
...
function solution(arr, n) {
  const result = arr.map((num, index) => {
    if (arr.length % 2 === 1) {
      // arr의 길이가 홀수일 때, 짝수 인덱스에 n을 더함
      return index % 2 === 0 ? num + n : num;
    } else {
      // arr의 길이가 짝수일 때, 홀수 인덱스에 n을 더함
